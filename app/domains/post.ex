defmodule App.Domains.Post do
  ...
  object :post do
    field :id, :id do
      ...
      resolve fn id, _, _ ->
        App.Lib.MultiBatch.batch([{&App.Models.Post.ids/1, id}], fn (batch_results) ->
    ...
    field :user, :user do
      ...
      resolve fn id, _, _ ->
        ...
        App.Lib.MultiBatch.batch([{&App.Models.Post.ids/1, id}, {&App.Models.User.ids/1, :user_id}], fn (batch_results) ->
          {:ok, batch_results |> Map.get(id) |> Map.get(:id)}
        end)
      end
    end
